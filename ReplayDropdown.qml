...
// Plugin copy of qs.Ui.Dropdown. Omarchy's popup uses CloseOnPressOutside,
// so a second click on the trigger closes then opens again.
// qs.Ui is required here: the original Dropdown lives in that module, so
// BorderSurface is in-scope there without an import. Copied out, the panel
// fails to instantiate with "Type ReplayDropdown unavailable".
//
  ...
  Column {
    ...
    BorderSurface {
      ...
      Popup {
        ...
        focus: true
        // Press on the trigger is outside the popup overlay, so the default
        // CloseOnPressOutside closes first and the click then opens again.
        // CloseOnPressOutsideParent still dismisses clicks elsewhere.
